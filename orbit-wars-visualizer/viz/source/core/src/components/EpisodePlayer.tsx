...
export function EpisodePlayer<TSteps extends BaseGameStep[] = BaseGameStep[]>({
  ...
  const playbackHandlersRef = useRef<{ onPlay?: () => boolean | void; onPause?: () => void }>({});
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Check if mouse is within container or container has focus
      // In notebooks, global listeners are dangerous, so we check if the container
      // or its children are the target or if the mouse is hovering.
      const isWithin = containerRef.current?.contains(target);
      const isHovering = containerRef.current?.matches(':hover');
      
      if (!isWithin && !isHovering) return;
  ...
  const handleRegisterPlaybackHandlers = useCallback((handlers: { onPlay?: () => boolean | void; onPause?: () => void }) => {
